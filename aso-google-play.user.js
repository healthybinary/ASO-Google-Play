...
/**
 * Scrape developer AppStoreSpy
 */
function getAppStoreSpyDeveloper(developer_name, developer_id) {
	return new Promise((resolve, reject) => {
		getHttpRequest(`https://appstorespy.com/developers/play/${developer_name}/${developer_id}`).then(response => {
			if(response.status !== 200) {
				resolve({
					status: response.status,
					app: null
			...
			tableElem.querySelectorAll('tbody tr').forEach(elem => {
				const tdTxt = elem.innerText;
				let thTxt = elem.querySelector('th');
				if(thTxt != null) {
					thTxt = thTxt.innerText.trim();
					if(tdTxt.indexOf('Total apps') > -1) {
						app.total = thTxt;
					} else if(tdTxt.indexOf('Removed apps') > -1) {
						app.remove = thTxt;
					} else if(tdTxt.indexOf('Age') > -1) {
						app.age = thTxt;
					} else if(tdTxt.indexOf('Total installs') > -1) {
						app.installs = thTxt;
					} else if(tdTxt.indexOf('Last updated') > -1) {
						app.last_update = elem.querySelector('td').innerText.trim();
					} else if(tdTxt.indexOf('1st App Released') > -1) {
						app.release_date = elem.querySelector('td').innerText.trim();
					}
				}
			})
			resolve({
...
function generateASO() {
	...
	const developer_id = document.querySelector('h1').parentElement.parentElement.querySelector('a').href.split('=')[1];
	const developer_name = document.querySelector('h1').parentElement.parentElement.querySelector('a').innerText.replace(/[\s-_]+/g, '');
	...
	if(app_data != null) {
		...
		// Get the developer
		if(app_data.app_store_dev != null) {
			app_data.app_store_dev.is_cached = true
			request_data.push(Promise.resolve(app_data.app_store_dev));
		} else {
			request_data.push(getAppStoreSpyDeveloper(developer_name, developer_id));
		}
	} else {
		request_data.push(scrapeAppBrain(package_name));
		request_data.push(scrapeAppStoreSpy(package_name));
	...
	Promise.all(request_data).then(values => {
		let [ app_brain, app_store, app_store_dev ] = values;

		if(app_brain.status === 200 || app_store.status === 200 || app_store_dev.status === 200) {
...
function initAddToListJS() {
	document.querySelector('body').insertBefore(evaluateHtml(aso_add_later_code), document.querySelector('body').children[document.querySelector('body').children.length-1]);
        ...
        if(clearCacheBtn != null) {
            ...
            document.querySelector('#clear-cache-appstore-dev').addEventListener('click', function(e) {
                e.preventDefault();
                const current_data = GM_getValue(`cache_${package_name}`);
                current_data.app_store_dev = null;
                GM_setValue(`cache_${package_name}`, current_data);
                document.querySelector('#cache-notice .content').innerHTML = `The AppStoreSpy Developer cache has been purged for "${package_name}" from the Tampermonkey database, next page will go straight to the AppStoreSpy Developer webpage.`;
            });
...
function injectData(app_data) {
	...
	const {app_brain, app_store, app_store_dev} = app_data;

	const app_brain_data = app_brain.data;
	const app_store_data = app_store.data;
	const app_store_dev_data = app_store_dev.data;
	...
	if(app_store_data != null) {
		...
		switch(app_store.status) {
			case 404:
				errors.push(`We have recieved a 404 erro page not found when requesting AppStoreSpy.Com!`);
				break;
			default:
				errors.push(`Looks like there was a problem when requesting the AppStoreSpy.Com Page ${app_store.status} Error Status!`);
				break;
		}
	...
	// App Store
	if(app_store_dev_data != null) {
		let is_app_data_exists = false;
		for (let property in app_store_dev_data.app) {
			const currentVal = app_store_dev_data.app[property];
			const selector = document.querySelector(`#app-data .content-appstore-dev .top-app-${property}`);
			if (currentVal != null && currentVal != '') {
				selector.querySelector(".value").innerHTML = app_store_dev_data.app[property];
				is_app_data_exists = true;
			} else {
				selector.parentElement.parentElement.removeChild(selector.parentElement);
			}
		...
		// Notice
		if (!is_app_data_exists) {
			document.querySelector('#app-data .content-appstore-dev').innerHTML = `The IP has been hit by rate limiting please try again later or use a proxy on browser`;
		}

		if(app_store_dev.is_cached) {
			is_results_cached = true;
			document.querySelector('#cache-notice #clear-cache-appstore-dev').style.display = 'inline-block';
		}
	} else {
		switch(app_store_dev.status) {
			case 404:
				errors.push(`We have recieved a 404 erro page not found when requesting AppStoreSpy.Com!`);
				break;
			default:
				errors.push(`Looks like there was a problem when requesting the AppStoreSpy.Com Page ${app_store_dev.status} Error Status!`);
				break;
		}

		["app-data"].forEach(selectorName => {
			const selector = document.querySelector(`#${selectorName} .content-appstore-dev`);
			if (selector != null ) {
				selector.innerHTML = `There was an error when the page has been request, the page returns ${app_store_dev.status} Status Code!`;
...
function scriptUpdateChecker() {
	getHttpRequest('https://raw.githubusercontent.com/ayoubfletcher/ASO-Google-Play/master/version.txt').then(res => {
		if(res.status === 200) {
			const versionScript = res.responseText.trim();
			const currentVersion = GM_info.script.version;
			const selector = document.querySelector('#update-notice');
			selector.querySelector('.old-version').innerHTML = currentVersion;
			selector.querySelector('.last-version').innerHTML = versionScript;
			if(versionScript !== currentVersion) {
