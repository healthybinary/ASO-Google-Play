		...
		getHttpRequest(`https://www.appbrain.com/app/${package_name}`).then(response => {
			...
			let app = {
				app_data: {
					...
					maturity: extractField(
						data.querySelector('.infotile[tooltip="Google Play content rating."]')
					)
				},
...
function extractGooglePlayRankings(resultDom) {
	const selector = resultDom.querySelector('.aso-data-table-container');
	if (selector == null) {
...
function injectData(app_data) {
	...
	if (app_brain_data != null) {
		...
		if (!is_app_data_exists) {
			document.querySelector('#app-data .content').innerHTML = `The IP has been hit by rate limiting please try again later or use a proxy on browser`;
		}
