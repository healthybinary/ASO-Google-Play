...
const app_brain_limit_error = `Looks like you've hit the daily pageview limit for AppBrain. The script detected the error on the AppBrain page. It gathered what it can from the page. to get the full data please consider changing the IP by restarting the router or using a VPN or maybe a proxy. Feel free to contact me or opening an issue on <a href="https://github.com/ayoubfletcher/ASO-Google-Play">Github Repo</a> if had any issue regarding the script.`;
let error_msg = "";
...
function generateASO() {
	...
	const regxDev = document.body.innerHTML.match(/ratings"\]\]\],\[\["(\d+)"/);
	if(regxDev.length == 2) {
		developer_id = regxDev[1];
	}
...
function extractKeywords(resultDom) {
	...
	let error_msg = `Sorry, no keywords has been found on AppBrain page.`;
	if (selector == null) {
		return error_msg;
	}
	let tags_html = selector.innerHTML.trim();
	if(tags_html == '') {
		tags_html = error_msg;
	}
	return tags_html;
